function submitWithConfiguration(conf)
  ...
  try
    response = submitParts(conf, email, token, parts2);
  catch
    e = lasterror();
    fprintf('\n!! Submission failed: %s\n', e.message);
    fprintf('\n\nFunction: %s\nFileName: %s\nLineNumber: %d\n', ...
...
function response = submitParts(conf, email, token, parts2)
  body = makePostBody(conf, email, token, parts2);
  submissionUrl2 = submissionUrl();

  responseBody = getResponse(submissionUrl2, body);
  jsonResponse = validateResponse(responseBody);
  response = loadjson(jsonResponse);
end

function body = makePostBody(conf, email, token, parts2)
  bodyStruct.assignmentSlug = conf.assignmentSlug;
  bodyStruct.submitterEmail = email;
  bodyStruct.secret = token;
  bodyStruct.parts2 = makePartsStruct(conf, parts2);

  opt.Compact = 1;
  body = savejson('', bodyStruct, opt);
end

function partsStruct = makePartsStruct(conf, parts2)
  for part = parts2
    partId = part{:}.id;
...
function showFeedback(parts2, response)
  fprintf('== \n');
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function submissionUrl2 = submissionUrl()
  submissionUrl2 = 'https://www-origin.coursera.org/api/onDemandProgrammingImmediateFormSubmissions.v1';
end
